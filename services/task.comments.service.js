...
module.exports = {
	...
	settings: {
		rest: "/",
		fields: ["_id", "author", "text", "task", "createdAt", "updatedAt"],
		entityValidator: {
	...
	actions: {
		...
		create: {
			...
			async handler(ctx) {
				...
				const isAuthorized = await ctx.call("user.isAuthorized", {
					id: ctx.meta.user._id,
					actionRank: "admin|project_manager|employee",
				});
				if (!isAuthorized)
					throw new MoleculerClientError(
						"User with that role can't use this action.",
						405
					);

				if (entity.author) {
					const isUserExisting = await ctx.call("user.isCreated", {
						id: entity.author,
					});
					if (!isUserExisting)
						throw new MoleculerClientError(
				...
				const isTaskExisting = await ctx.call("tasks.isCreated", {
					id: entity.task,
				});
				if (!isTaskExisting)
					throw new MoleculerClientError(
						"Provided task not found!",
						404
					);
				...
				this.broker.emit("task.comment.created", {
					comment: doc._id,
					task: entity.task,
				});
		...
		update: {
			...
			async handler(ctx) {
				...
				const isAuthorized = await ctx.call("user.isAuthorized", {
					id: ctx.meta.user._id,
					actionRank: "admin|project_manager",
				});
				if (!isAuthorized)
					throw new MoleculerClientError(
						"User with that role can't use this action.",
		...
		get: {
			...
			async handler(ctx) {
				...
				const comment = await this.adapter.findById(this.adapter.stringToObjectID(ctx.params.id));
				if (comment) {
					const author = await ctx.call("user.getBasicData", {
						id: comment.author,
		...
		remove: {
			...
			async handler(ctx) {
				...
				this.broker.emit("task.comment.removed", {
					comment: comment._id,
					task: comment.task,
				});
		...
		"user.removed": {
			async handler(payload) {
				...
				while (idx--) {
					...
					this.adapter.updateById(comments[idx]._id, {
						$set: comments[idx],
					});
				}
			},
